...
沒有資料庫。第一屏把你的頭貼、自介、對外連結和引言排在乾淨的版面上；往下捲還有
第二屏 About 動畫，依序說完四段文字，最後收成一個手寫簽名。
...
| `gsap` | 動畫引擎 | 第一屏進場、頭貼波動框、About 時間軸和 SVG 筆畫 |
| `@gsap/react` | GSAP 官方的 React 綁定 | 提供 `useGSAP()`，元件消失時自動清掉動畫 |
| `matter-js` | 2D 物理引擎 | 計算 About 每個單字掉落時的位置、旋轉與碰撞 |
...
| `@types/react`、`@types/react-dom`、`@types/node`、`@types/matter-js` | 幫這些函式庫補上型別說明書 |
...
src/main.tsx                     ← 程式的入口（entry point）
   ...
   │  2. resetInitialScroll()    → 清除舊捲動位置，固定從第一屏開始
   │  3. 用 site.meta 改 document.title / description
   │  4. createRoot(#root).render(<App />)
   ▼
src/App.tsx                      ← 版面總指揮：串起兩個滿版區塊
   │
   ├─▶ 第一屏 hub
   │    ├─▶ ProfileHeader ──▶ Avatar ──▶ avatarRing.ts
   │    ├─▶ LinkList ──▶ LinkButton ──▶ icons/registry
   │    ├─▶ QuoteBlock / IconLinkRow
   │    ├─▶ ScrollCue（通往 About 的圓點）
   │    └─▶ playEntrance()（整屏進場）
   │
   └─▶ AboutSection
        └─▶ aboutReveal.ts ──▶ wordPhysics.ts ──▶ matter-js
```

`index.html` 裡的 `<div id="root"></div>` 是空的。**整個網站的 HTML 都是 JavaScript
在瀏覽器裡生出來的**，這就是所謂的 SPA（single-page application）。這裡的「一頁」
指只有一個網址和入口；裡面仍然可以有兩個滿版 `<section>`。

`src/initialScroll.ts` 在 React 掛載前把 `history.scrollRestoration` 設為 `manual`、移除網址的
fragment（例如 `#about`），並立即捲到 `(0, 0)`。它還會在 `pageshow` 再置頂一次，防止瀏覽器
稍後用舊捲動位置覆蓋第一次設定。因此即使你在 About 重新整理，畫面也固定回到整頁最上方的
Linktree；React 接著掛載第一屏，`playEntrance()` 會從起點重新播放。這不是直接顯示動畫結束
狀態，而是重現第一次開啟網址的流程。同一次瀏覽中點擊 `ScrollCue` 仍會正常前往 About。
...
```ts
export type { About, Avatar, LinkGroup, LinkItem, Profile, Quote, SiteConfig, SiteMeta } from './types'
...
```
               ...
               ┌─────────────────┼────────────┬──────────────┐
               ▼                 ▼            ▼              ▼
        initialScroll.ts     index.css      App.tsx    data/index.ts (site)
                                 │
       ┌─────────────┬───────────┼───────────┬──────────────┐
       ▼             ▼           ▼           ▼              ▼
 ProfileHeader    LinkList   QuoteBlock  IconLinkRow   AboutSection
       │             │                       │              │
       ▼             ▼                       │              ▼
    Avatar       LinkButton ◀────────────────┘       aboutReveal.ts
       │             │                                      │
       ▼             ▼                                      ▼
 avatarRing.ts  icons/registry.tsx                    wordPhysics.ts
                     ▲                                      │
                     │                                      ▼
                data/types.ts                           matter-js
                     ▲
       ┌─────────────┼──────────────┐
       │             │              │
  profile.ts     links.ts       about.ts
       └─────────────┼──────────────┘
                     ▼
                  site.ts       selectors.ts
                     └──────┬──────┘
                            ▼
                      data/index.ts
```
...
第 2 層  data/profile|links|about  只 import types
第 3 層  data/site|selectors   組合第 2 層
第 4 層  data/index.ts         門面
第 5 層  animation/*、components/*  消費 DOM、資料或圖示，不定義網站內容
第 6 層  App.tsx               只負責組合兩屏與排版順序
第 7 層  main.tsx              掛載
```

**沒有循環依賴**（A import B、B 又 import A）。這不是巧合，是設計出來的：
資料不知道元件的存在，元件不知道版面的存在。
因此資料不需要知道元件如何呈現，元件也不需要知道整頁如何排列；修改時的影響範圍較小。
...
| 檔案 | 做什麼 |
|---|---|
| `index.html` | 唯一 HTML。載 Google Fonts（Inter + Newsreader + EB Garamond）、favicon、`<div id="root">` |
...
| `test/` | Node 內建測試；驗證重新整理回到第一屏，以及背景分頁恢復後才播放進場 |
| `public/` | **原封不動被複製到 `dist/`**。`avatar.webp`、`favicon.svg` 在這裡。程式裡用 `/avatar.webp` 引用（開頭的 `/` 代表網站根目錄） |
| `src/assets/` | 跟 `public/` 不同：這裡的東西要被 import 才會進打包，會加上 hash 檔名。包含 portfolio 原稿、About 標題和收尾簽名 SVG |
...
- `About` — 第二屏的朗讀標題和依播放順序排列的 `paragraphs`
...
export const about: About = {
  heading: 'About me.',
  paragraphs: [/* 四段依播放順序排列的文字 */],
}
```

`heading` 是給螢幕閱讀器的真正標題；看得見的版本來自 SVG。`paragraphs` 的陣列順序
就是動畫順序。斷行由版面和 `SplitText` 決定，不要在字串裡手動插入換行。
...
export const site: SiteConfig = {
  ...
  profile, groups, footerLinks, about,
...
const hub = useRef<HTMLElement>(null)            // 只抓第一屏
useGSAP(() => playEntrance(hub.current), { scope: hub })
```

`useRef` 是 React 的「幫我記住那個真正的 DOM 元素」。GSAP 要動的是真實 DOM，
所以必須拿到它。`ref={hub}` 掛上去之後，`hub.current` 就是第一屏 `<section>`。
About 有自己的 `useGSAP`，不跟著第一屏進場。
...
整個引言區塊就消失，不會爆錯。`about && ...` 同理：移除 `site.about` 後，提示圓點和
第二屏會一起消失，網站會退回單屏。
...
### 9.8 `ScrollCue.tsx` — 第一屏底部的提示

它是連到 `#about` 的 `<a>`，而不是只有 JavaScript 才能用的按鈕。中間可見的藍點只有
7px，但外層點擊區是 44px；鍵盤、觸控和 JavaScript 關閉時仍然能使用。瀏覽器允許動態
效果時，CSS 讓它輕微呼吸，點擊後平滑捲到第二屏。

### 9.9 `AboutSection.tsx` — 第二屏版面

這個元件只負責 DOM 結構和響應式尺寸，動畫交給 `playAboutReveal()`：

- 真正的 `<h2>` 用 `sr-only` 保留給螢幕閱讀器；可見標題是裝飾性 SVG。
- 兩份 SVG 用 Vite 的 `?raw` 內嵌，DrawSVG 才能直接操作每一條 `<path>`。
- 四個 `<p>` 疊在同一個 grid 格子，確保每段從相同位置出現。
- `[data-stage]` 是掉落單字的座標原點。
- 收尾簽名用 absolute 固定在區塊中央；`overflow-hidden` 裁掉落出第二屏的字。

桌面尺寸主要用 `vw` / `dvh` 維持設計稿比例；`sm` 以下回到適合手機閱讀的單欄。
...
瀏覽器在背景分頁會把 `requestAnimationFrame` 降頻。如果載入時直接建立 tween，動畫可能
在人看不到時跑完；如果直接 `return`，又無法保證重新整理後重播。現在的做法是：頁面可見就
立即開始，隱藏時先監聽 `visibilitychange`，等重新可見後移除監聽器並從第一格開始播放。
...
**注意 `App.tsx` 只把第一屏 `<section>` 一個元素交給它**，所以第一屏整塊一起動、沒有先後順序
（這是你當初明確要求的「不要有順序」）。
...
gsap.to(wave, { angle: `+=${TAU * TURNS}`, duration: MORPH * TURNS, ease: 'none', repeat: -1 })
```

`write()` 把 `angle` 換算成八個百分比。每條邊的兩個半徑固定加總 100%，四條邊則使用
不同相位與權重。主波在完整週期走 `TURNS = 3` 圈，副波走互質的 `SUB = 2` 圈，
`SKEW` 決定副波比例；因此每一圈輪廓都不同，到第三圈才無縫回到起點。
...
主要常數：`MORPH` 每圈秒數、`TURNS` / `SUB` 兩條波的圈數、`SKEW` 副波比例、
`AMP` 形變幅度，以及 `ENTER` / `SETTLE` 的進出場時間。
...
### 10.4 `aboutReveal.ts` + `wordPhysics.ts` — About 動畫

整段不是一串互不相干的 timeout，而是一條可暫停、重建和重播的 GSAP timeline：
...
界線後會從物理世界移除並隱藏，避免持續運算或撐大頁面。

重置不能只把 timeline 拉回 0：Matter.js 直接寫過 `position`、`left`、`top`、
`transform` 和 `visibility`。`reset()` 會清掉所有這些 inline style，再讓下一輪從原本
排版開始。元件卸載時還會 kill timeline / ScrollTrigger、銷毀物理引擎並 revert SplitText。

### 10.5 `useGSAP()` 為什麼一定要用
...
`scope` 限定選擇器只在這個元素內找。`playEntrance()` 另外回傳 cleanup，確保尚未觸發的
`visibilitychange` 監聽器和自己的 `matchMedia` 都會被移除。
...
這是重點：換配色只改這裡，元件一行都不用動。`--accent` 是全站唯一彩色，連接第一屏
的提示圓點與第二屏的手寫 SVG。
...
@theme inline {
  ...
  --font-garamond: 'EB Garamond', ui-serif, Georgia, serif;
...
`--font-garamond` 則產生按鈕標題使用的 `font-garamond`。
...
About 的 `.signature svg` 統一接管寬度與 overflow，`.signature path` 把兩份原始 SVG 的
筆畫改成 `--accent`。`.scroll-cue` 使用 `cue-breathe` keyframes；只有允許動態效果時，
`html` 才開啟平滑捲動。
...
| 改 About 文字或順序 | `src/data/about.ts` 的 `paragraphs` |
| 暫時拿掉整個 About | 移除 `src/data/site.ts` 的 `about` 欄位 |
| 換 About 手寫標題 / 收尾簽名 | `src/assets/about-signature.svg` / `src/assets/signature.svg` |
...
這四段是這個專案裡真的踩過坑、然後想清楚的地方。看不懂可以先跳過。
...
`47.5px` 由圓點 40px 加上兩側各 3.75px 內縮而來。內縮值與位移公式必須一起改，
否則圓點停下的位置會偏離另一側。

### 13.3 About 如何協調時間軸、DOM 和物理世界

這段動畫同時有三套狀態：GSAP timeline 的時間、瀏覽器 DOM 的實際位置、Matter.js body
的物理位置。它們不能互相假設對方會自動還原。

1. `SplitText` 先依目前字型與寬度產生單字元素。
2. `measure()` 在所有字仍位於正常排版時記下邊界和 stage 高度。
3. timeline 到達段落的 `fallAt` 時才呼叫 `physics.drop(words)`。
4. `drop()` 必須先量完整批次，再把字改成 absolute；逐字邊量邊改會讓後面的字補位，
   所有起點因此偏掉。
5. Matter.js 後續直接同步 `left`、`top` 和旋轉；越界後移除 body 並隱藏 DOM。

重切行是另一個難點。`SplitText` 會在字型載入或寬度改變時重建單字，因此程式先 kill
舊 timeline、重新量測，再依 `revealed` 決定回到起點或直接快進結局。物理引擎沒有
`progress(1)`，所以 `settle()` 用有限步數模擬到單字全部越界。
...
| **tween** | 一段從 A 到 B 的動畫 |
| **timeline** | 按時間安排多個 tween 與回呼的序列 |
| **ease** | 動畫的速度曲線 |
| **GSAP plugin** | 加在 GSAP 上的專門能力；本專案使用 DrawSVG、SplitText、ScrollTrigger |
| **physics body** | Matter.js 世界裡代表一個單字的矩形物體 |
...
2. **履歷連結已經指向 `/resume.pdf`，但 `public/` 還沒有這個檔案**。在檔案補上以前，
   點擊 Résumé 會得到 404。
